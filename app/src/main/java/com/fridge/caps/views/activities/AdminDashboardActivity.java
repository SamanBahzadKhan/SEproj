...
public class AdminDashboardActivity extends AppCompatActivity {
    ...
    private static final int      REQ_ADD_COUNSELOR = 1001;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnAddCounselor.setOnClickListener(v ->
                startActivityForResult(
                        new Intent(this, AddCounselorActivity.class),
                        REQ_ADD_COUNSELOR));
    ...
    private void loadAll() {
        ...
        appointmentController.getAllBookedTimeslots(new AppointmentController.AppointmentListCallback() {
            ...
            public void onSuccess(List<Appointment> appointments) {
                if (!canUpdateUi()) return;
                try {
                    int n = appointments != null ? appointments.size() : 0;
                    if (tvTotalAppts != null) {
                        tvTotalAppts.setText(String.valueOf(n));
                    }
                } catch (Throwable t) {
                    Log.e(TAG, "appointments count UI", t);
                } finally {
                    if (progressBar != null) {
        ...
        counselorController.getAllCounselorsForAdmin(new CounselorController.CounselorListCallback() {
            ...
            public void onSuccess(List<Counselor> counselors) {
                if (!canUpdateUi()) return;
                try {
                    List<Counselor> safeCounselors = counselors != null ? counselors : new ArrayList<>();
                    if (tvTotalCounselors != null) {
                        tvTotalCounselors.setText(String.valueOf(safeCounselors.size()));
                    }
                    if (rvCounselors != null) {
                        rvCounselors.setAdapter(new CounselorAdapter(safeCounselors,
                                c -> {
                                    String id = c != null ? c.getUserId() : null;
                    ...
                    Log.e(TAG, "counselors list UI", t);
                }
            }
            @Override public void onFailure(String e) {}
        });
    ...
    private void listenPendingReports() {
        reportListener = reportController.listenPendingReports(new ReportController.ReportListCallback() {
            ...
            public void onSuccess(List<UserReport> reports) {
                if (!canUpdateUi()) return;
                try {
                    List<UserReport> safeReports = reports != null ? reports : new ArrayList<>();
                    int pendingCount = safeReports.size();
                    if (tvReportsCount != null) {
                        tvReportsCount.setText(formatStatCount(pendingCount));
                    }
                    updateAdminBellBadge(pendingCount);
                    if (tvNoReports != null) {
                        tvNoReports.setVisibility(safeReports.isEmpty() ? View.VISIBLE : View.GONE);
                    }
                    if (rvAppointments != null) {
                        rvAppointments.setAdapter(new com.fridge.caps.views.adapters.ReportedUserAdapter(
                                safeReports, new com.fridge.caps.views.adapters.ReportedUserAdapter.ActionListener() {
                            @Override
                    ...
                    Log.e(TAG, "reports listener UI", t);
                }
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQ_ADD_COUNSELOR && resultCode == RESULT_OK) {
            loadAll();
        }
    }
